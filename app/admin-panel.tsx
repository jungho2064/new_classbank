...
export default function AdminPanel({ supabase, userList, transactions, seats, fundData, isFrozen, depositOpen, loadData, showAlert, onLogout }: any) {
  ...
  const handleResolvePending = async (tId: number, status: 'Success' | 'Rejected', name: string, isWithdrawal: boolean) => {
    ...
    await loadData();
    showAlert(`요청이 [${status === 'Success' ? '승인' : '거절'}] 처리되었습니다.`);
  };
  ...
  const handleRewardPenalty = async () => {
    ...
    await loadData();
    showAlert(`🏆 ${rewardTarget} 대원에게 ${rewardType} ${amt}안 반영 완료!`);
  };
  ...
  const handlePaySalaries = async () => {
    ...
    await loadData();
    showAlert('💸 전 대원 주급 지급 및 세금/대출 자동 정산이 완료되었습니다!');
  };

  // 4. 특례 대출 발급
  const handleIssueLoan = async () => {
    const amt = parseInt(loanAmt);
    if (!loanTarget || isNaN(amt) || amt <= 0) { showAlert('⚠️ 학생과 대출금을 확인하세요.'); return; }
  ...
  const handleUpdateGlobalRate = async () => {
    ...
    await supabase.from('system_config').upsert({ key: 'weekly_loan_rate', value: String(rate) });
    await supabase.from('users').update({ individual_rate: 0 }).neq('id', 0);
    await loadData();
    showAlert(`✅ 일괄 이율 ${rate}%가 적용되었으며 개별 이율은 초기화되었습니다.`);
  };
  ...
  const handleCollectRent = async () => {
    const nowStr = new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' });
    const rows: any[] = [];
    for (const s of seats.filter((seat: any) => seat.owner)) {
      rows.push({ date: nowStr, name: s.owner, type: '임대료 납부', amount: -Number(s.rent), note: `${s.seat}번 좌석`, status: 'Success' });
      rows.push({ date: nowStr, name: '국고(중앙은행)', type: '임대료 수입', amount: Number(s.rent), note: `${s.owner} 임대료`, status: 'Success' });
    }
    if (rows.length > 0) await supabase.from('transactions').insert(rows);
    await loadData();
    showAlert('💸 모든 입주 학생의 임대료 징수가 완료되었습니다!');
  };
  ...
  const handleResetEstateSeason = async () => {
    ...
    await loadData();
    showAlert('🔄 거품 방지 공식이 적용되어 모든 좌석의 새 시즌 가격이 책정되었습니다.');
  };

  // 7. 만기 예금 일괄 지급 (이자소득세 15% 징수)
  const handleMatureAllDeposits = async () => {
    const nowStr = new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' });
    const today = new Date().toISOString().split('T')[0];
    const activeDeposits = transactions.filter((t: any) => t.status === 'Deposit_Active');
    const rows: any[] = [];
    ...
    for (const d of activeDeposits) {
      try {
        ...
        if (parts['만기'] <= today) {
          const prin = parseInt(parts['원금']);
          const rate = parseInt(parts['이율']);
  ...
  const handleUpdateFundIndex = async () => {
    ...
    await loadData();
    showAlert(`📈 펀드 지수가 [${newIdx}p]로 업데이트되었습니다!`);
  };
  ...
  const handleBatchFundBuy = async () => {
    ...
    for (const u of approved) {
      const myBal = transactions.filter((t: any) => t.name === u.name && t.status !== 'Rejected' && t.status !== 'Deposit_Active').reduce((a: any, c: any) => a + Number(c.amount), 0);
      if (myBal >= amt) {
    ...
    if (rows.length > 0) await supabase.from('transactions').insert(rows);
    await loadData();
    showAlert(`🚀 ${rows.length / 2}명의 대원이 펀드에 자동 가입되었습니다!`);
  };

  // 국고 및 통화량 산출
  const treasuryBal = transactions.filter((t: any) => t.name === '국고(중앙은행)' && t.status === 'Success').reduce((a: any, c: any) => a + Number(c.amount), 0);
  const totalMoneySupply = transactions.filter((t: any) => t.name !== '국고(중앙은행)' && t.status !== 'Rejected' && t.status !== 'Deposit_Active').reduce((a: any, c: any) => a + Number(c.amount), 0);
      ...
      <header className="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-30 flex justify-between items-center max-w-5xl mx-auto">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">국고 잔액: {treasuryBal.toLocaleString()}안 | 통화량: {totalMoneySupply.toLocaleString()}안</p>
          </div>
        </div>
        ...
        {adminTab === 'pending' && (
          <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
              ...
              {userList.filter((u: any) => u.status === 'Pending').length === 0 ? <p className="text-xs text-slate-500">대기 중인 가입 신청이 없습니다.</p> : userList.filter((u: any) => u.status === 'Pending').map((u: any) => (
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
              ...
              {transactions.filter((t: any) => t.status === 'Pending_W').length === 0 ? <p className="text-xs text-slate-500">대기 중인 출금 요청이 없습니다.</p> : transactions.filter((t: any) => t.status === 'Pending_W').map((t: any) => (
        ...
        {adminTab === 'reward' && (
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-xs">
              <select value={rewardTarget} onChange={e => setRewardTarget(e.target.value)} className="bg-slate-950 border border-slate-800 p-3 rounded-xl">
              ...
              <input type="number" placeholder="금액 (안)" value={rewardAmt} onChange={e => setRewardAmt(e.target.value)} className="bg-slate-950 border border-slate-800 p-3 rounded-xl" />
              <input type="text" placeholder="사유 (예: 환경정리 우수)" value={rewardReason} onChange={e => setRewardReason(e.target.value)} className="bg-slate-950 border border-slate-800 p-3 rounded-xl" />
            </div>
        ...
        {adminTab === 'salary' && (
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div><label className="text-slate-400">유지비 (%)</label><input type="number" value={maintRate} onChange={e => setMaintRate(Number(e.target.value))} className="w-full bg-slate-950 p-2.5 rounded-xl border border-slate-800 mt-1"/></div>
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <select value={loanTarget} onChange={e => setLoanTarget(e.target.value)} className="bg-slate-950 border border-slate-800 p-3 rounded-xl">
                  <option value="">학생 선택</option>
                  {userList.filter((u: any) => u.status === 'Approved').map((u: any) => <option key={u.id} value={u.name}>{u.name}</option>)}
                </select>
                <input type="number" placeholder="대출 원금" value={loanAmt} onChange={e => setLoanAmt(e.target.value)} className="bg-slate-950 border border-slate-800 p-3 rounded-xl" />
                <input type="number" placeholder="주당 이율(%)" value={loanRate} onChange={e => setLoanRate(e.target.value)} className="bg-slate-950 border border-slate-800 p-3 rounded-xl" />
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
              ...
              <div className="flex gap-2 text-xs">
                <input type="number" value={globalLoanRate} onChange={e => setGlobalLoanRate(e.target.value)} className="flex-1 bg-slate-950 border border-slate-800 p-3 rounded-xl" placeholder="새 일괄 이율(%)"/>
        ...
        {adminTab === 'store' && (
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
              <input type="text" placeholder="아이템 이름" value={newItemName} onChange={e => setNewItemName(e.target.value)} className="bg-slate-950 p-2.5 rounded-xl border border-slate-800" />
              <input type="number" placeholder="가격 (안)" value={newItemPrice} onChange={e => setNewItemPrice(e.target.value)} className="bg-slate-950 p-2.5 rounded-xl border border-slate-800" />
              <input type="number" placeholder="재고 수량" value={newItemStock} onChange={e => setNewItemStock(e.target.value)} className="bg-slate-950 p-2.5 rounded-xl border border-slate-800" />
              <input type="text" placeholder="설명" value={newItemDesc} onChange={e => setNewItemDesc(e.target.value)} className="bg-slate-950 p-2.5 rounded-xl border border-slate-800" />
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div><label className="text-slate-400">상승 점수 가산(+)</label><input type="number" value={posScore} onChange={e => setPosScore(Number(e.target.value))} className="w-full bg-slate-950 p-2.5 rounded-xl border border-slate-800 mt-1"/></div>
                <div><label className="text-slate-400">하락 점수 감산(-)</label><input type="number" value={negScore} onChange={e => setNegScore(Number(e.target.value))} className="w-full bg-slate-950 p-2.5 rounded-xl border border-slate-800 mt-1"/></div>
              </div>
              <input type="text" placeholder="어제의 뉴스" value={fundNews} onChange={e => setFundNews(e.target.value)} className="w-full bg-slate-950 p-2.5 rounded-xl border border-slate-800 text-xs" />
              <input type="text" placeholder="오늘의 이모저모 힌트" value={fundHint} onChange={e => setFundHint(e.target.value)} className="w-full bg-slate-950 p-2.5 rounded-xl border border-slate-800 text-xs" />
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
              ...
              <div className="flex gap-2">
                <input type="number" placeholder="1인당 가입액" value={batchFundAmt} onChange={e => setBatchFundAmt(e.target.value)} className="flex-1 bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs" />
            ...
            <div className="space-y-1.5 max-h-96 overflow-y-auto">
              {transactions.map((t: any) => (
                <div key={t.id} className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 text-[11px] flex justify-between">
                  ...
                  <span className={`font-bold ${Number(t.amount) > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{t.amount}안</span>
                </div>
              ))}
            </div>
          </div>
